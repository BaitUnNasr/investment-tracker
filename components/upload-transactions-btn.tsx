...
function validateTransactionRow(row: Omit<TransactionRow, "errors">): string[] {
  const errs: string[] = []
  if (!DATE_RE.test(row.date))              errs.push("Invalid date (expected DD/MM/YYYY)")
  if (!row.accountNo)                       errs.push("A/C No is missing")
  if (row.credit === 0 && row.debit === 0)  errs.push("Amount is zero")
  if (!row.dc || !/^(cr|dr)/i.test(row.dc)) errs.push("D/C must be Cr or Dr")
  return errs
}

function parseAndValidateCsv(text: string): TransactionRow[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    rowNum++
    const debit  = parseFloat((cols[8]  ?? "").replace(/,/g, "")) || 0
    ...
    const base = {
      ...
      debit,
      credit,
      dc: cols[11] ?? (credit > 0 ? "Cr" : "Dr"),
    }
    rows.push({ ...base, errors: validateTransactionRow(base) })
  }
  return rows
}
...
export function UploadTransactionsBtn() {
  ...
  function handleFileSelect(f: File) {
    ...
    reader.onload = (e) => {
      setRows(parseAndValidateCsv(e.target?.result as string))
      setStep("preview")
    }
    reader.readAsText(f)
  }
      ...
      >
        <DialogContent className="flex max-h-[90vh] w-[90vw] max-w-[90vw] sm:max-w-[90vw] flex-col gap-0 p-0">
          ...
          <DialogHeader className="border-b px-6 py-4">
            ...
            <DialogDescription>
              Expected columns: Date · Trans Id · Type · Rc/Vc No · A/C No · Chq No · GL Name ·
              Narration · Debit · Credit · Balance · DC
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-6 py-4">
            ...
            {!file ? (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">Bank statement format (.csv)</p>
              </button>
            ) : (
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10 bg-muted/90 backdrop-blur-sm">
                    <tr>
                      ...
                      <th className="px-3 py-2.5 text-left font-medium">Trans ID</th>
                      <th className="px-3 py-2.5 text-left font-medium">A/C No</th>
                      <th className="px-3 py-2.5 text-left font-medium">Type</th>
                      <th className="px-3 py-2.5 text-left font-medium">Narration</th>
                      <th className="px-3 py-2.5 text-right font-medium">Amount (₹)</th>
                      <th className="px-3 py-2.5 text-left font-medium">D/C</th>
                    ...
                    {rows.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 font-mono">{r.transId}</td>
                          <td className={`px-3 py-2 font-mono ${!r.accountNo ? "text-red-600" : ""}`}>
                            {r.accountNo || <span className="italic text-muted-foreground">missing</span>}
                          </td>
                          ...
                          <td className={`px-3 py-2 text-right font-medium ${
                            r.credit === 0 && r.debit === 0 ? "text-red-600" : ""
                          }`}>
                            {(r.credit || r.debit) > 0
                              ? (r.credit || r.debit).toLocaleString("en-IN")
                              : <span className="italic text-muted-foreground">0</span>
                            }
                          </td>
                          <td className={`px-3 py-2 ${
                            r.dc && /^(cr|dr)/i.test(r.dc) ? "" : "text-red-600"
                          }`}>
                            {r.dc || <span className="italic text-muted-foreground">—</span>}
                          </td>
            ...
            {step === "preview" && rows.length === 0 && (
              <div className="flex items-center gap-2 rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm dark:border-yellow-800 dark:bg-yellow-950/30">
                ...
                No data rows found. Verify the file has the correct 12-column format.
              </div>
            )}
